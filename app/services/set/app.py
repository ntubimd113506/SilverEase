...
def setting():
    ...
    cursor = conn.cursor()
    old = cursor.execute(
        'SELECT FamilyID FROM Family where MainUserID = %s', (MemID))
...
def CodeID():
    ...
    cursor.execute(
        'SELECT FamilyID FROM FamilyCode WHERE CodeID = %s', (CodeID,))
    res = cursor.fetchone()
    ...
    cursor.execute(
        'SELECT * FROM FamilyLink WHERE FamilyID = %s AND SubUserID = %s', (FamilyID, MemID))
    che = cursor.fetchone()
    ...
    if che:
        ...
        cursor.execute(
            'INSERT INTO FamilyLink (FamilyID, SubUserID) VALUES (%s, %s)', (FamilyID, MemID))
        cursor1.execute(
            'SELECT MainUserID FROM Family WHERE FamilyID = %s', (FamilyID,))
        old1 = cursor1.fetchone()
        oldID = old1[0]
        cursor2.execute(
            'SELECT MemName FROM Member WHERE MemID = %s', (oldID,))
...
def family_list():
    conn = db.get_connection()
    cursor = conn.cursor()
    cursor.execute('SELECT * FROM Family WHERE MainUserID = %s',
                   (session.get("MemID"),))           
    res = cursor.fetchone()
    MainFamily = {cursor.description[i][0]: res[i]
                  for i in range(len(res))} if res else None
    SubFamilys = []
    cursor.execute(
        'SELECT * FROM Family WHERE FamilyID in (SELECT FamilyID FROM FamilyLink WHERE SubUserID = %s)', (session.get("MemID"),))
    for res in cursor.fetchall():
        SubFamilys.append(
            {cursor.description[i][0]: res[i] for i in range(len(res))})
    conn.close()

    if MainFamily:
        session["MainFamilyID"] = MainFamily["FamilyID"]
        MainInfo = line_bot_api.get_profile(
            MainFamily["MainUserID"]).as_json_dict()
        MainFamily["Name"] = MainInfo["displayName"]
        MainFamily["Picture"] = MainInfo["pictureUrl"]
   
    for SubFamily in SubFamilys:
        SubInfo = line_bot_api.get_profile(
            SubFamily["MainUserID"]).as_json_dict()
...
def delete_family():
    conn = db.get_connection()
    cursor = conn.cursor()
    cursor.execute('Delete FROM Family WHERE FamilyID = %s',(session.get("MainFamilyID"),))
    conn.commit()
    conn.close()
...
def access_check():
    ...
    if session.get("FamilyID"):
        conn=db.get_connection()
        cur=conn.cursor()
        cur.execute('SELECT * FROM Access WHERE FamilyID = %s', (session.get("FamilyID"),))
        res=cur.fetchone()
        conn.close()
...
def access_submit():
    ...
    cur = conn.cursor()
    if session.get("FamilyID"):
        cur.execute('UPDATE Access SET GPS = %s, DataAnalyze=%s WHERE FamilyID = %s',
    ...
    if request.form.get('option') == 'old':
        ...
        while 1:
            ...
            if data != None:
                ...
                conn.commit()
                cursor1.execute(
                    'INSERT INTO Family (MainUserID) VALUES (%s)', (MemID))
                conn.commit()
...
def checkid():
    try:
        ...
        # ==================  ==================
        cursor.execute(
            'SELECT MainUserID FROM Family WHERE MainUserID = %s', (MemID))
        member_data = cursor.fetchone()
        cursor.execute(
            'SELECT SubUserID FROM FamilyLink WHERE SubUserID = %s', (MemID))
