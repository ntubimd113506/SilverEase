...
class Config:
    ...
    MQTT_TLS_CA_CERTS = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "ca", "my-ca.crt"
    )
    MQTT_TLS_VERSION = 5
    ...
    SCHEDULER_JOBSTORES = {
        "default": SQLAlchemyJobStore(
            url=f'sqlite:///{os.path.join(os.path.dirname(os.path.abspath(__file__)),"services","scheduler","jobstore","scheduler.db")}'
        )
    }
